-- Correção do ranking: LEAST() do Postgres ignora NULL, então LEAST(NULL, 100)
-- virava 100 e atletas SEM avaliação apareciam no topo com nota máxima.
-- Agora a nota da avaliação só é usada quando existe.
...
BEGIN
  ...
  scored AS (
    SELECT b.*,
      ...
      CASE WHEN _skill IS NULL AND un.nota_geral IS NOT NULL THEN LEAST(un.nota_geral * 20, 100) END AS aval_score
    FROM base b
