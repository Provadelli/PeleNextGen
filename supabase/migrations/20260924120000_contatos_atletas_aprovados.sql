-- 1) contatos_desbloqueados.candidato_id pode ser candidatos.id OU o profiles.id
--    do atleta aprovado por avaliação direta (sem linha em candidatos), como já
--    assume clube_has_unlocked_atleta(). A FK original impedia esse segundo caso.
ALTER TABLE public.contatos_desbloqueados
  DROP CONSTRAINT IF EXISTS contatos_desbloqueados_candidato_id_fkey;

-- 2) Atletas aprovados por avaliação direta: vale a ÚLTIMA avaliação com
--    decisão. Quem foi aprovado e depois reprovado deixa de aparecer.
...
BEGIN
  ...
  ultima_decisao AS (
    ...
    WHERE a.decisao IS NOT NULL
      AND a.atleta_user_id IS NOT NULL
    ORDER BY a.atleta_user_id, a.created_at DESC
  ),
  aval_aprov AS (
    SELECT ud.user_id, ud.nota_geral
    FROM ultima_decisao ud
    WHERE ud.decisao = 'aprovado'
      AND NOT EXISTS (SELECT 1 FROM cand_aprov ca WHERE ca.user_id = ud.user_id)
  )
